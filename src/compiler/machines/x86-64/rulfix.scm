  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-2-ARGS PLUS-FIXNUM
			  ...
			  (OBJECT->FIXNUM (CONSTANT (? n)))
			  #f)))
  (add-immediate-and-tag target source n))
  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-1-ARG ONE-PLUS-FIXNUM
			 (REGISTER (? source))
			 #f)))
  (add-immediate-and-tag target source 1))
...
(define (add-immediate-and-tag target source n)
  (fixnum-1-arg target source
    (lambda (target)
	...
	(define (general)
	  (with-signed-immediate-operand n
	    (lambda (operand)
	      (LAP (ADD Q ,target ,operand)))))
	(LAP ,@(cond ((fits-in-signed-byte? n) (add))
