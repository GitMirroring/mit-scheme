  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-2-ARGS PLUS-FIXNUM
			  ...
			  (REGISTER (? source2))
			  #f)))
  ;; If no overflow detection needed, use LEA for four-operand addition
  ;; to add the two sources and the fixnum tag into a possibly separate
  ;; target in one swell foop.
  ;;
  ;; XXX Might be nice to take advantage of the scale too when one of
  ;; the operands is multiplied by 2/4/8 or shifted by 1/2/3, but that
  ;; requires a lot of intermediate rules.
  (let* ((source1 (source-register source1))
	 (source2 (source-register source2))
	 (target (target-register target)))
    (LAP (LEA Q (R ,target) (@ROI ,source1 ,type-code:fixnum ,source2 1))
  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-2-ARGS FIXNUM-XOR
			  ...
			  (OBJECT->FIXNUM (CONSTANT (? n)))
			  #f)))
  (QUALIFIER (<= 0 n))			;Must not toggle tag.
  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-2-ARGS FIXNUM-AND
			  ...
			  (OBJECT->FIXNUM (CONSTANT (? n)))
			  #f)))
  (QUALIFIER (< n 0))		    ;Sign-extension must not clear tag.
  ...
  (ASSIGN (REGISTER (? target))
	  (FIXNUM->OBJECT
	   (FIXNUM-2-ARGS FIXNUM-ANDC
			  ...
			  (OBJECT->FIXNUM (CONSTANT (? n)))
			  #f)))
  (QUALIFIER (<= 0 n))		    ;Sign-extension must not clear tag.
