...
static inline void
push_reflection (unsigned long code, tctx_t* tctx)
{
  stack_push (ULONG_TO_FIXNUM (code), tctx);
  stack_push (reflect_to_interface, tctx);
}
...
static long link_remaining_sections (link_cc_state_t*, tctx_t*);
static void start_linking_cc_block (tctx_t*);
static void end_linking_cc_block (link_cc_state_t*);
static void restore_linking_cc_block_p (void*);
static void update_cache_after_link (link_cc_state_t*);
static void start_linking_section (link_cc_state_t*);
static long link_section (link_cc_state_t*, tctx_t*);
static bool link_section_handler
  (linkage_section_type_t, cache_handler_t**, bool*);
static void back_out_of_link_section (link_cc_state_t*, tctx_t*);
static void restore_link_cc_state (link_cc_state_t*, SCHEME_OBJECT, tctx_t*);
...
{                                                                       \
  insn_t* ep_ = (ep);                                                   \
  return C_to_interface (CC_ENTRY_ADDRESS_PTR (ep_),                    \
                         CC_ENTRY_ADDRESS_PC (ep_));                    \
} while (false)

#define ENTER_SCHEME_CONTINUATION(c) do                                 \
{                                                                       \
  insn_t* c_ = (c);                                                     \
  return C_to_interface (CC_RETURN_ADDRESS_PTR (c_),                    \
                         CC_RETURN_ADDRESS_PC (c_));                    \
} while (false)
...
#define SAVE_LAST_RETURN_CODE_PUSHES 2

static inline void
save_last_return_code (unsigned long rc, tctx_t* tctx)
{
  unsigned long offset = stack_pointer (tctx) - last_return_code (tctx);
...
static inline void
check_last_return_code (tctx_t* tctx)
{
  assert (RETURN_CODE_P (*last_return_code (tctx)));
}

static inline void
restore_last_return_code (SCHEME_OBJECT offset, tctx_t* tctx)
{
  set_last_return_code (stack_loc (FIXNUM_TO_ULONG (offset), tctx), tctx);
...
void
compiler_initialize (bool fasl_p, tctx_t* tctx)
{
  ...
  if (fasl_p)
    compiler_reset (make_compiler_utilities (tctx), tctx);
  else
    {
...
make_compiler_utilities (tctx_t* tctx)
{
  SCHEME_OBJECT h1;
  SCHEME_OBJECT h2;
...

  {
	...
	Microcode_Termination (TERM_COMPILER_DEATH, tctx);
	/*NOTREACHED*/
      }
  }
...
void
compiler_reset (SCHEME_OBJECT new_block, tctx_t* tctx)
{
...
long
enter_compiled_expression (SCHEME_OBJECT exp, SCHEME_OBJECT env, tctx_t* tctx)
{
  cc_entry_type_t cet;
  if (read_cc_entry_type (&cet, CC_ENTRY_ADDRESS (exp)))
  ...
  guarantee_cc_return (0, tctx);
  SET_CC_ENV (env);
  JUMP_TO_CC_ENTRY (exp);
}
...
long
return_to_compiled_code (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  ...
  if (CC_RETURN_P (cont))
    ...
    bad:
      ...
      push_cont_rc (RC_REENTER_COMPILED_CODE, offset, tctx);
  ...
  if (stack_ref (offset, tctx) == MAKE_RETURN_CODE (RC_REENTER_COMPILED_CODE))
    {
      ...
      set_last_return_code (stack_loc (offset + lrc, tctx), tctx);
      check_last_return_code (tctx);
      compiler_end_subproblem (tctx);
    }
  else
    {
      set_last_return_code (stack_loc (offset, tctx), tctx);
      check_last_return_code (tctx);
...
static inline void
invoke_return_address (utility_result_t* DSU_result, tctx_t* tctx)
{
  if (Free >= GET_MEMTOP)
    {
      compiler_interrupt_return_to_entry (tctx);
      compiler_interrupt_common (DSU_result, 0, GET_CC_VAL, tctx);
      return;
    }
  assert (CC_RETURN_P (stack_ref (0, tctx)));
  RETURN_TO_SCHEME_CONTINUATION (CC_RETURN_ADDRESS (stack_pop (tctx)));
}

#define TAIL_CALL_1(pname, a1, tctx) do                                 \
{                                                                       \
  pname (DSU_result, (unsigned long) (a1), 0, 0, 0, (tctx));            \
  return;                                                               \
} while (false)

#define TAIL_CALL_2(pname, a1, a2, tctx) do                             \
{                                                                       \
  pname (DSU_result,                                                    \
	 (unsigned long) (a1),                                          \
	 (unsigned long) (a2),                                          \
...
DEFINE_SCHEME_UTILITY_2 (comutil_apply, procedure, frame_size)
{
  ...
  while (1)
	...
	{
	  long code
            = (setup_compiled_invocation (procedure, frame_size - 1, tctx));
	  if (code != PRIM_DONE)
	    RETURN_TO_C (code);
	}
	...
	if (IMPLEMENTED_PRIMITIVE_P (procedure))
	  {
	    ...
	    if (arity == frame_size - 1)
	      TAIL_CALL_1 (comutil_primitive_apply, procedure, tctx);
	    else if (arity == LEXPR)
	      {
                set_primitive_lexpr_actuals (frame_size - 1, tctx);
		TAIL_CALL_1 (comutil_primitive_lexpr_apply, procedure, tctx);
	      }
	    else
	      {
		recover_from_apply_error (procedure, frame_size - 1, tctx);
...
DEFINE_SCHEME_UTILITY_2 (comutil_lexpr_apply, address, n_args)
{
  ...
  if (! ((!read_cc_entry_type ((&cet), address))
    ...
    {
      recover_from_apply_error (MAKE_CC_ENTRY (address), n_args, tctx);
...
DEFINE_SCHEME_UTILITY_1 (comutil_primitive_apply, primitive)
{
  ...
  apply_primitive_external (primitive, tctx);
  POP_PRIMITIVE_FRAME (PRIMITIVE_ARITY (primitive));
  invoke_return_address (DSU_result, tctx);
}
...
DEFINE_SCHEME_UTILITY_1 (comutil_primitive_lexpr_apply, primitive)
{
  DECLARE_UTILITY_ARG (SCHEME_OBJECT, primitive);
  apply_primitive_external (primitive, tctx);
  POP_PRIMITIVE_FRAME (primitive_lexpr_actuals (tctx));
  invoke_return_address (DSU_result, tctx);
}
			 ...
			 n_sections)
{
  ...
  start_linking_cc_block (tctx);
  long result = link_remaining_sections (&s, tctx);
...
long
comp_link_caches_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  link_cc_state_t s;
  restore_link_cc_state (&s, offset, tctx);
  SET_CC_ENV (cc_block_environment (MAKE_CC_BLOCK (s.block_address)));

  start_linking_cc_block (tctx);

  long result = (link_section (&s, tctx));
  if (result == PRIM_DONE)
    result = (link_remaining_sections (&s, tctx));
...
static long
link_remaining_sections (link_cc_state_t* s, tctx_t* tctx)
{
...
static void
start_linking_cc_block (tctx_t* tctx)
{
  abort_linking_t* al = dstack_alloc (sizeof (abort_linking_t));
  al->saved_linking = linking_cc_block_p (tctx);
  al->tctx = tctx;
  transaction_begin ();
  transaction_record_action (tat_always, restore_linking_cc_block_p, al);
  set_linking_cc_block_p (true, tctx);
}
...
static void
restore_linking_cc_block_p (void* ap)
{
  abort_linking_t* al = (abort_linking_t*) ap;
  set_linking_cc_block_p (al->saved_linking, al->tctx);
}
...
static long
link_section (link_cc_state_t* s, tctx_t* tctx)
{
  ...
  while (n_linked < n_entries)
    {
      result = (*handler) (execute_p ? read_uuo_symbol (scan) : *scan,
  ...
  *s->scan0 = make_linkage_section_marker (s->type, n_linked);
  if (result == PRIM_DONE)
    ...
    back_out_of_link_section (s, tctx);
  return (result);
}

static bool
...
static void
back_out_of_link_section (link_cc_state_t* s, tctx_t* tctx)
{
  /* Save enough state to restart.  */
  stack_push (MAKE_CC_RETURN (s->return_address), tctx);
  stack_push (ULONG_TO_FIXNUM (s->n_sections - s->n_linked_sections), tctx);
  stack_push (ULONG_TO_FIXNUM (s->scan0 - s->block_address), tctx);
  stack_push (ULONG_TO_FIXNUM (s->scan - s->block_address), tctx);
  stack_push (MAKE_CC_BLOCK (s->block_address), tctx);
  stack_push (ULONG_TO_FIXNUM (s->n_entries - s->n_linked_entries), tctx);
  stack_push (ULONG_TO_FIXNUM (s->n_entries), tctx);
  save_last_return_code (RC_COMP_LINK_CACHES_RESTART, tctx);
}

static void
restore_link_cc_state (link_cc_state_t* s, SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  s->n_entries = OBJECT_DATUM (stack_pop (tctx));
  s->n_linked_entries = s->n_entries - OBJECT_DATUM (stack_pop (tctx));
  s->block_address = OBJECT_ADDRESS (stack_pop (tctx));
  s->scan = s->block_address + OBJECT_DATUM (stack_pop (tctx));
  s->scan0 = s->block_address + OBJECT_DATUM (stack_pop (tctx));
  s->n_sections = OBJECT_DATUM (stack_pop (tctx));
  s->return_address = CC_RETURN_ADDRESS (stack_pop (tctx));
  s->n_linked_sections = 0;
  s->type = linkage_section_type (*s->scan0);
}

SCHEME_OBJECT
cc_block_linkage_info (SCHEME_OBJECT block, tctx_t* tctx)
{
  SCHEME_OBJECT* const_addr
    = VECTOR_LOC
        (block,
         1
         + VECTOR_LENGTH (MAKE_POINTER_OBJECT (TC_VECTOR,
                                               VECTOR_LOC (block, 0))));
  SCHEME_OBJECT* block_end = CC_BLOCK_END (block);
  unsigned long n_sections;
  unsigned long n_words;
  count_linkage_sections (const_addr, block_end, (&n_sections), (&n_words));
  primitive_gc_if_needed (n_words, tctx);
...
{
  DECLARE_UTILITY_ARG (insn_t*, entry_point);
  compiler_interrupt_common (DSU_result, entry_point, SHARP_F, tctx);
}

/* GET_CC_VAL has live data, and there is no entry address on the stack */

DEFINE_SCHEME_UTILITY_1 (comutil_interrupt_continuation, entry_point)
{
  DECLARE_UTILITY_ARG (insn_t*, entry_point);
  compiler_interrupt_common (DSU_result, entry_point, GET_CC_VAL, tctx);
}

/* GET_CC_ENV has live data; no entry point on the stack */

DEFINE_SCHEME_UTILITY_1 (comutil_interrupt_ic_procedure, entry_point)
{
  DECLARE_UTILITY_ARG (insn_t*, entry_point);
  compiler_interrupt_common (DSU_result, entry_point, GET_CC_ENV, tctx);
}

/* GET_CC_VAL has live data, and the tagged continuation is on top of stack.  */
...
static void
compiler_interrupt_return_to_entry (tctx_t* tctx)
{
  SCHEME_OBJECT ret = stack_pop (tctx);
  assert (CC_RETURN_P (ret));
  insn_t* ret_addr = CC_RETURN_ADDRESS (ret);
  insn_t* entry_addr = CC_RETURN_ADDRESS_TO_ENTRY_ADDRESS (ret_addr);
  stack_push (MAKE_CC_ENTRY (entry_addr), tctx);
}

void
compiler_interrupt_common (utility_result_t* DSU_result,
			   insn_t* entry_address,
			   SCHEME_OBJECT val_or_env,
                           tctx_t* tctx)
{
  if (!FREE_OK_P (Free))
    request_gc (Free - heap_alloc_limit, tctx);
  stack_check (2 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
  if (entry_address != 0)
    stack_push (MAKE_CC_ENTRY (entry_address), tctx);
  else
    assert (CC_ENTRY_P (stack_ref (0, tctx)));
  stack_push (val_or_env, tctx);
  save_last_return_code (RC_COMP_INTERRUPT_RESTART, tctx);
  RETURN_TO_C (PRIM_INTERRUPT);
}

long
comp_interrupt_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  // Saved object could be either val or env, so set both.
  SCHEME_OBJECT val_or_env = stack_pop (tctx);
  SET_CC_VAL (val_or_env);
  SET_CC_ENV (val_or_env);
  JUMP_TO_CC_ENTRY (stack_pop (tctx));
}

/* Other traps */

static void
setup_lookup_restart (long code, insn_t* ret_addr, SCHEME_OBJECT cache,
                      unsigned long rc, tctx_t* tctx)
{
  SCHEME_OBJECT sra = MAKE_CC_RETURN (ret_addr);
  SCHEME_OBJECT block = cc_return_to_block (sra);
  stack_check (3 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
  stack_push (sra, tctx);
  stack_push (cc_block_environment (block), tctx);
  stack_push (compiler_var_error (cache, block, CACHE_REFERENCES_LOOKUP), tctx);
  save_last_return_code (rc, tctx);
}
  ...
  if (code != PRIM_DONE)
    {
      SCHEME_OBJECT sra = (MAKE_CC_RETURN (ret_addr));
      SCHEME_OBJECT block = (cc_return_to_block (sra));
      stack_check (4 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
      stack_push (sra, tctx);
      stack_push (new_val, tctx);
      stack_push (cc_block_environment (block), tctx);
      stack_push
	(compiler_var_error (cache, block, CACHE_REFERENCES_ASSIGNMENT), tctx);
      save_last_return_code (RC_COMP_ASSIGNMENT_TRAP_RESTART, tctx);
      RETURN_TO_C (code);
    }
  SET_CC_VAL (old_val);
...
long
comp_assignment_trap_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  SCHEME_OBJECT name = stack_ref (0, tctx);
  SCHEME_OBJECT environment = stack_ref (1, tctx);
  SCHEME_OBJECT new_val = stack_ref (2, tctx);
  SCHEME_OBJECT old_val;
  ...
  increment_sp (3, tctx);
  SET_CC_VAL (old_val);
  JUMP_TO_CC_RETURN (stack_pop (tctx));
}

  ...
  if (code != PRIM_DONE)
    {
      ...
      stack_check (4 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
      stack_push (block, tctx);
      stack_push (ULONG_TO_FIXNUM (frame_size), tctx);
      stack_push (cc_block_environment (block), tctx);
      stack_push
	(compiler_var_error (cache, block, CACHE_REFERENCES_OPERATOR), tctx);
      save_last_return_code (RC_COMP_CACHE_REF_APPLY_RESTART, tctx);
      RETURN_TO_C (code);
    }
  TAIL_CALL_2 (comutil_apply, value, frame_size, tctx);
}

long
comp_cache_lookup_apply_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  SCHEME_OBJECT name = stack_ref (0, tctx);
  SCHEME_OBJECT environment = stack_ref (1, tctx);
  SCHEME_OBJECT frame_size = stack_ref (2, tctx);
  SCHEME_OBJECT value;
  ...
  increment_sp (4, tctx);
  stack_push (value, tctx);
  stack_push (make_apply_frame_header (FIXNUM_TO_ULONG (frame_size)), tctx);
  if (CC_ENTRY_P (value))
    return apply_compiled_procedure (tctx);
  guarantee_interp_return (tctx);
  return PRIM_APPLY;
}

...
DEFINE_SCHEME_UTILITY_2 (comutil_lookup_trap, ret_addr, cache_addr)
{
  ...
  if (code != PRIM_DONE)
    {
      setup_lookup_restart
        (code, ret_addr, cache, RC_COMP_LOOKUP_TRAP_RESTART, tctx);
      RETURN_TO_C (code);
    }
  SET_CC_VAL (val);
  RETURN_TO_SCHEME_CONTINUATION (ret_addr);
...
long
comp_lookup_trap_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  SCHEME_OBJECT name = stack_ref (0, tctx);
  SCHEME_OBJECT environment = stack_ref (1, tctx);
  ...
  if (code != PRIM_DONE)
    {
      save_last_return_code (RC_COMP_LOOKUP_TRAP_RESTART, tctx);
      return (code);
    }
  increment_sp (2, tctx);
  SET_CC_VAL (val);
...
DEFINE_SCHEME_UTILITY_2 (comutil_safe_lookup_trap, ret_addr, cache_addr)
{
  ...
  if (code != PRIM_DONE)
    {
      setup_lookup_restart
        (code, ret_addr, cache, RC_COMP_SAFE_REF_TRAP_RESTART, tctx);
      RETURN_TO_C (code);
    }
  SET_CC_VAL (val);
  RETURN_TO_SCHEME_CONTINUATION (ret_addr);
...
long
comp_safe_lookup_trap_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  SCHEME_OBJECT name = stack_ref (0, tctx);
  SCHEME_OBJECT environment = stack_ref (1, tctx);
  ...
  if (code != PRIM_DONE)
    {
      save_last_return_code (RC_COMP_SAFE_REF_TRAP_RESTART, tctx);
      return (code);
    }
  increment_sp (2, tctx);
  SET_CC_VAL (val);
...
DEFINE_SCHEME_UTILITY_2 (comutil_unassigned_p_trap, ret_addr, cache_addr)
{
  ...
  if (code != PRIM_DONE)
    {
      setup_lookup_restart
        (code, ret_addr, cache, RC_COMP_UNASSIGNED_TRAP_RESTART, tctx);
      RETURN_TO_C (code);
    }
  SET_CC_VAL (val);
  RETURN_TO_SCHEME_CONTINUATION (ret_addr);
...
long
comp_unassigned_p_trap_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  SCHEME_OBJECT name = stack_ref (0, tctx);
  SCHEME_OBJECT environment = stack_ref (1, tctx);
  ...
  if (code != PRIM_DONE)
    {
      save_last_return_code (RC_COMP_UNASSIGNED_TRAP_RESTART, tctx);
      return code;
    }
  SET_CC_VAL (val);
  JUMP_TO_CC_RETURN (stack_pop (tctx));
...
{                                                                       \
  TAIL_CALL_2                                                           \
    (comutil_apply,                                                     \
     VECTOR_REF (fixed_objects, (fobj_index)),                          \
     (arity),                                                           \
     tctx);                                                             \
}
...
DEFINE_SCHEME_UTILITY_2 (comutil_primitive_error, ret_addr, primitive)
{
  ...
  DECLARE_UTILITY_ARG (SCHEME_OBJECT, primitive);
  stack_check (2 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
  stack_push (MAKE_CC_ENTRY (ret_addr), tctx);
  stack_push (primitive, tctx);
  save_last_return_code (RC_COMP_ERROR_RESTART, tctx);
  RETURN_TO_C (ERR_COMPILED_CODE_ERROR);
}

long
comp_error_restart (SCHEME_OBJECT offset, tctx_t* tctx)
{
  restore_last_return_code (offset, tctx);
  increment_sp (1, tctx);       /* primitive */
  JUMP_TO_CC_ENTRY (stack_pop (tctx));
}

void
                               ...
                               tctx_t* tctx)
{
  ...
  while (true)
	...
	{
	case TC_COMPILED_ENTRY:
	  setup_compiled_invocation_from_primitive (procedure, n_args, tctx);
	  return;
	...
	default:
	  ...
	  push_reflection (REFLECT_CODE_INTERNAL_APPLY, tctx);
	  return;
	}
    }
}
...
compiled_with_interrupt_mask (unsigned long old_mask,
			      ...
			      unsigned long new_mask,
                              tctx_t* tctx)
{
  stack_push (ULONG_TO_FIXNUM (old_mask), tctx);
  push_reflection (REFLECT_CODE_RESTORE_INTERRUPT_MASK, tctx);
  stack_push (ULONG_TO_FIXNUM (new_mask), tctx);
  setup_compiled_invocation_from_primitive (receiver, 1, tctx);
}

void
compiled_with_stack_marker (SCHEME_OBJECT thunk, tctx_t* tctx)
{
  push_reflection (REFLECT_CODE_STACK_MARKER, tctx);
  setup_compiled_invocation_from_primitive (thunk, 0, tctx);
}
                                          ...
                                          tctx_t* tctx)
{
  ...
  if (code != PRIM_DONE)
	...
	{
	  set_prim_apply_error_code (code, tctx);
	  code = PRIM_APPLY_ERROR;
	}
      PRIMITIVE_ABORT (code);
    }
  stack_push (procedure, tctx);
  push_reflection (REFLECT_CODE_COMPILED_INVOCATION, tctx);
}

                           ...
                           tctx_t* tctx)
{
  ...
  if (read_cc_entry_type ((&cet), (CC_ENTRY_ADDRESS (procedure))))
    {
      recover_from_apply_error (procedure, n_args, tctx);
      return (ERR_COMPILED_CODE_ERROR);
    }
  ...
  if (cet.args.for_procedure.rest_p)
    return (setup_lexpr_invocation (procedure, n_args, n_max, tctx));
  if (n_args == n_max)
    return (PRIM_DONE);
  if (n_args > n_max)
    {
                        ...
                        tctx_t* tctx)
{
  if (n_args <= n_max)
      ...
      if (open_gap (n_args, n_max + 1, tctx))
	{
      ...
      stack_set (n_max, EMPTY_LIST, tctx);
      return PRIM_DONE;
    }
  {
  ...
  }
  {
    ...
    SCHEME_OBJECT* p1 = stack_loc (n_max, tctx);
    for (unsigned long i = n_max; (i < n_args); i += 1)
      {
        *Free++ = *p1++;
        *Free = MAKE_POINTER_OBJECT (TC_LIST, Free + 1);
        Free += 1;
      }
    Free[-1] = EMPTY_LIST;
    *--p1 = rest_arg;
    {
      SCHEME_OBJECT* p2 = stack_loc (n_max, tctx);
      for (unsigned long i = 0; (i < n_max); i += 1)
        *--p1 = *--p2;
    }
    set_stack_pointer (p1, tctx);
  }
  return PRIM_DONE;
}
...
open_gap (unsigned long n_args, unsigned long n_needed, tctx_t* tctx)
{
  ...
  stack_check (n_defaults, tctx);

  open_stack_gap (n_args, n_defaults, tctx);

  SCHEME_OBJECT* scan = stack_loc (n_args, tctx);
  SCHEME_OBJECT* end = scan + n_defaults;
  while (scan < end)
    *scan++ = DEFAULT_OBJECT;
  return false;
}
...
{
  SCHEME_OBJECT code = stack_pop (tctx);
  ...
  switch (OBJECT_DATUM (code))
    ...
    case REFLECT_CODE_INTERNAL_APPLY:
      {
	unsigned long frame_size = (OBJECT_DATUM (stack_pop (tctx)));
	SCHEME_OBJECT procedure = stack_pop (tctx);
	TAIL_CALL_2 (comutil_apply, procedure, frame_size, tctx);
      }

    case REFLECT_CODE_RESTORE_INTERRUPT_MASK:
      SET_INTERRUPT_MASK (OBJECT_DATUM (stack_pop (tctx)));
      invoke_return_address (DSU_result, tctx);
      /*NOTREACHED*/
      break;

    case REFLECT_CODE_STACK_MARKER:
      increment_sp (2, tctx);   // two markers
      invoke_return_address (DSU_result, tctx);
      /*NOTREACHED*/
      break;
    ...
    case REFLECT_CODE_CC_BKPT:
      ...
      if (Free >= GET_MEMTOP)
	{
	  push_reflection (REFLECT_CODE_CC_BKPT, tctx);
	  compiler_interrupt_common (DSU_result, 0, SHARP_F, tctx);
	  return;
	}
      {
	insn_t* addr;
	long code = do_bkpt_proceed (&addr, tctx);
	if (code != PRIM_DONE)
	  {
	    stack_push (code, tctx);
	    RETURN_TO_C (code);
	  }
...
DEFINE_TRAMPOLINE (comutil_operator_apply_trap)
{
  ...
  TAIL_CALL_2 (comutil_apply, procedure, OBJECT_DATUM (frame_header), tctx);
}

DEFINE_TRAMPOLINE (comutil_operator_primitive_trap)
{
  INIT_TRAMPOLINE_1 (primitive);
  TAIL_CALL_1 (comutil_primitive_apply, primitive, tctx);
}

DEFINE_TRAMPOLINE (comutil_operator_lexpr_trap)
{
  INIT_TRAMPOLINE_2 (procedure, frame_header);
  set_primitive_lexpr_actuals (apply_frame_header_n_args (frame_header), tctx);
  TAIL_CALL_1 (comutil_primitive_lexpr_apply, procedure, tctx);
}

...
DEFINE_TRAMPOLINE (comutil_operator_1_0_trap)
{
  ...
  stack_push (DEFAULT_OBJECT, tctx);
  RETURN_TO_SCHEME_ENTRY (CC_ENTRY_ADDRESS (procedure));
}
...
{
  INIT_TRAMPOLINE_1 (procedure);
  SCHEME_OBJECT a1 = stack_pop (tctx);
  stack_push (DEFAULT_OBJECT, tctx);
  stack_push (a1, tctx);
  RETURN_TO_SCHEME_ENTRY (CC_ENTRY_ADDRESS (procedure));
}
...
DEFINE_TRAMPOLINE (comutil_operator_3_2_trap)
{
  ...
  {
    SCHEME_OBJECT a1 = stack_pop (tctx);
    SCHEME_OBJECT a2 = stack_pop (tctx);
    stack_push (DEFAULT_OBJECT, tctx);
    stack_push (a2, tctx);
    stack_push (a1, tctx);
  }
...
DEFINE_TRAMPOLINE (comutil_operator_4_3_trap)
{
  INIT_TRAMPOLINE_1 (procedure);
  {
    SCHEME_OBJECT a1 = stack_pop (tctx);
    SCHEME_OBJECT a2 = stack_pop (tctx);
    SCHEME_OBJECT a3 = stack_pop (tctx);
    stack_push (DEFAULT_OBJECT, tctx);
    stack_push (a3, tctx);
    stack_push (a2, tctx);
    stack_push (a1, tctx);
...
DEFINE_TRAMPOLINE (comutil_operator_lookup_trap)
{
  ...
  if (code != PRIM_DONE)
    {
      stack_check (4 + SAVE_LAST_RETURN_CODE_PUSHES, tctx);
      stack_push (MAKE_CC_ENTRY (read_uuo_target_no_reloc (cache_addr)), tctx);
      /* Next three for debugger.  */
      stack_push (ULONG_TO_FIXNUM (frame_size), tctx);
      stack_push (cc_block_environment (block), tctx);
      stack_push
	(compiler_var_error (cache, block, CACHE_REFERENCES_OPERATOR), tctx);
      save_last_return_code (RC_COMP_OP_REF_TRAP_RESTART, tctx);
      RETURN_TO_C (code);
    }
  TAIL_CALL_2 (comutil_apply, procedure, frame_size, tctx);
}
...
long
comp_op_lookup_trap_restart (SCHEME_OBJECT lr_offset, tctx_t* tctx)
{
  restore_last_return_code (lr_offset, tctx);
  // Discard debugger info.
  increment_sp (3, tctx);
  SCHEME_OBJECT* store
    = trampoline_storage (cc_entry_to_block_address (stack_pop (tctx)));
		 ...
		 ...)
{
  ...
  if (GC_NEEDED_P (n_words))
    {
      request_gc (n_words, current_tctx ());
		       ...
		       unsigned long frame_size)
{
  cc_entry_type_t cet;
  make_cc_entry_type (&cet, CET_TRAMPOLINE);
  return make_trampoline
           (slot, &cet, kind, 2, procedure, ULONG_TO_FIXNUM (frame_size));
}

...
bkpt_proceed (insn_t* ep, SCHEME_OBJECT handle, SCHEME_OBJECT state,
              tctx_t* tctx)
{
  SCHEME_OBJECT ret = stack_ref (BKPT_PROCEED_FRAME_SIZE, tctx);
  if (! (CC_RETURN_P (ret) && CC_RETURN_ADDRESS (ret) == ep))
    error_external_return ();
  push_reflection (REFLECT_CODE_CC_BKPT, tctx);
  decrement_sp (BKPT_PROCEED_FRAME_SIZE, tctx);
  return SHARP_F;
}
...
long
do_bkpt_proceed (insn_t** addr_r, tctx_t* tctx)
{
...
DEFINE_SCHEME_UTILITY_2 (comutil_compiled_code_bkpt, entry_addr, state)
{
  ...
  stack_push (entry, tctx);
  SCHEME_OBJECT stack_ptr = MAKE_CC_STACK_ENV (stack_pointer (tctx));
  stack_push (to_save, tctx);
  stack_push (stack_ptr, tctx);
  stack_push (entry, tctx);
  TAIL_CALL_2 (comutil_apply,
...
declare_builtin (unsigned long builtin, const char* name)
{
  if (n_builtins == s_builtins)
      ...
      else
	{
	  ...
	  builtins = realloc (builtins, s_builtins * sizeof (unsigned long));
	  builtin_names = realloc (builtin_names, s_builtins * sizeof (char*));
	}
